 ...
 * 
 * PdxReader parses Paradox DB files.
 * Should work with versions 3 through 7.
 *
 * You can test the class from the command line:  java -jar PdxReader.jar <filename.db> 
 * This should echo all fields to the terminal.
 * 
 * Fileformat information: Randy Beck.
 * 
 * PdxReader is free software; you can redistribute it and/or modify
...
import nl.pwy.pdxreader.BigDate;
import nl.pwy.pdxreader.LEDataInputStream;
...
public class PdxReader {
    ...
    public static void main(String[] args) {
        ...
        String pdxFile = args[0];
        new PdxReader(pdxFile);
        System.exit(1);
    }
    ...
    private void tableWriter() {
        for (int j = 1; j <= numRecords + 1; j++) {
            for (int i = 1; i <= numFields; i++) {
                System.out.println(tableStringArray[i][j]);
            }
        }
    }
